...
AmbientLight	BlinnPhongMaterial::_a = AmbientLight(255, 255, 255, 0.2);
		...
		const PointLight &l)
{
	...
	Uint8 red = static_cast<Uint8>(
			std::clamp((ambient[0] + diffuse[0] + specular[0]), 0.0f, 255.0f));
	Uint8 green = static_cast<Uint8>(
			std::clamp((ambient[1] + diffuse[1] + specular[1]), 0.0f, 255.0f));
	Uint8 blue = static_cast<Uint8>(
			std::clamp((ambient[2] + diffuse[2] + specular[2]), 0.0f, 255.0f));
